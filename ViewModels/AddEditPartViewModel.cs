...
namespace ESCenter.ViewModels
{
    public class AddEditPartViewModel : ObservableObject
    {
        ...
        {
            var win = new Views.AddTypeDefinitionWindow(true) { Owner = Application.Current.Windows.OfType<Window>().FirstOrDefault(w => w.DataContext == this) };
        ...
        private void Save() { if (string.IsNullOrWhiteSpace(Part.PartType) || Part.PartType == "Other") { MessageBox.Show("Please select a valid part type."); return; } CloseWindow(true); }
